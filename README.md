...
2. Настройте параметры связи через регистры Modbus:
   - Адрес устройства (регистр 0x01)
   - Скорость обмена (регистр 0x02)
   - Таймаут ответа (регистр 0x03)

### Настройка сетевых интерфейсов
1. **Modbus**:
   - Используйте регистры 0x01-0x03 для настройки
   - По умолчанию: адрес 1, скорость 9600 бод

2. **СПТ**:
   - Адрес приемника (регистр 0x04)
   - Адрес источника (регистр 0x05) 
   - Скорость обмена (регистр 0x06)
   - Таймаут (регистр 0x07)

3. **WiFi**:
   - Режим работы (регистр 0x09)
   - Настройки точек доступа (регистры 0x20-0xDF)
