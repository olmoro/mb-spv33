...
### Важное замечание: Весь обмен по протоколу MODBUS идёт независимо от реакции целевого прибора, который 
    получив команду вернёт ответ в регистры, которые надо будет прочитать терминальной программой.

### Отправка пакета целевому прибору и чтение ответа (исходим из того, что в память прибора пакет 
    запроса заранее записан)
- Запишем в регистр 0x0B значение 0xFF20;
-  
